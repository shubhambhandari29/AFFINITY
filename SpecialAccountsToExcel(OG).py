...
#display(output_df[output_df['ALAE Paid'].apply(lambda x: pd.notnull(x) and float(x) % 1 != 0)])
...
keep_as_string = {"Customer Number", "Policy Number", "Policy MOD", "Claim Number", "Exposure",
                  "Producing Agent Code", "Policy Holder Claim Number", "Vehicle Year"}
...
for c in formatted_df.columns:
    if c not in keep_as_string:
        formatted_df[c] = pd.to_numeric(formatted_df[c], errors='ignore')
