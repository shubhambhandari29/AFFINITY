...
from services.sac import deduct_bill_distribution_service
...
def test_get_distribution_success(monkeypatch):
    ...
    monkeypatch.setattr(
        deduct_bill_distribution_service, "sanitize_filters", lambda params, allowed: params
    )
    monkeypatch.setattr(
        deduct_bill_distribution_service, "fetch_records_async", fake_fetch_records_async
    )
    monkeypatch.setattr(
        deduct_bill_distribution_service, "format_records_dates", lambda records: records
    )

    result = asyncio.run(
        deduct_bill_distribution_service.get_distribution({"CustomerNum": "1"})
    )
...
def test_get_distribution_invalid_filters(monkeypatch):
    ...
    monkeypatch.setattr(
        deduct_bill_distribution_service, "sanitize_filters", fake_sanitize_filters
    )

    with pytest.raises(HTTPException) as excinfo:
        asyncio.run(deduct_bill_distribution_service.get_distribution({"Bad": "X"}))
...
def test_upsert_distribution_strips_identity_columns(monkeypatch):
    ...
    monkeypatch.setattr(
        deduct_bill_distribution_service, "normalize_payload_dates", lambda payload: dict(payload)
    )
    monkeypatch.setattr(
    ...
    result = asyncio.run(deduct_bill_distribution_service.upsert_distribution(data_list))
...
def test_upsert_distribution_error(monkeypatch):
    ...
    with pytest.raises(HTTPException) as excinfo:
        asyncio.run(
            deduct_bill_distribution_service.upsert_distribution(
...
def test_delete_distribution_success(monkeypatch):
    ...
    monkeypatch.setattr(
        deduct_bill_distribution_service, "delete_records_async", fake_delete_records_async
    )

    data_list = [{"CustomerNum": "1", "AttnTo": "A"}]
    result = asyncio.run(deduct_bill_distribution_service.delete_distribution(data_list))
    assert result == {"count": 1}
