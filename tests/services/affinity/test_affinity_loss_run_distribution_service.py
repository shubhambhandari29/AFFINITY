...
from services.affinity import loss_run_distribution_service
...
def test_get_distribution_success(monkeypatch):
    ...
    monkeypatch.setattr(loss_run_distribution_service, "sanitize_filters", lambda params: params)
    monkeypatch.setattr(
        loss_run_distribution_service, "fetch_records_async", fake_fetch_records_async
    )
    monkeypatch.setattr(
        loss_run_distribution_service, "format_records_dates", lambda records: records
    )

    result = asyncio.run(loss_run_distribution_service.get_distribution({"ProgramName": "A"}))
...
def test_upsert_distribution_strips_identity_columns(monkeypatch):
    ...
    monkeypatch.setattr(
        loss_run_distribution_service, "normalize_payload_dates", lambda payload: dict(payload)
    )
    monkeypatch.setattr(
        loss_run_distribution_service, "merge_upsert_records_async", fake_merge_upsert_records_async
    )
...
def test_upsert_distribution_error(monkeypatch):
    ...
    with pytest.raises(HTTPException) as excinfo:
        asyncio.run(
            loss_run_distribution_service.upsert_distribution(
...
def test_delete_distribution_success(monkeypatch):
    ...
    monkeypatch.setattr(loss_run_distribution_service, "delete_records_async", fake_delete_records_async)
