...
async def get_distribution(query_params: dict):
    """
    Fetch account(s) from tblDist_ClaimRev_AFFIN.
    If query_params is provided, filters by given key/value.
    Returns a list of dicts (records).
    """

    try:
        base_query = "SELECT * FROM tblDist_ClaimRev_AFFIN"
        filters = []
        params = []
        ...
        logger.warning(f"Error fetching Claim Review Distribution List - {str(e)}")
...
async def upsert_distribution(data_list):
    ...
    Update row if already exists, else insert row into tblDist_ClaimRev_AFFIN
    """
    ...
    try:
        ...
        for data in data_list:
            ...
            merge_query = f"""
            MERGE INTO tblDist_ClaimRev_AFFIN AS target
...
async def delete_distribution(data_list):
    """
    Takes an array of maps as data
    Deletes matching rows from tblDist_ClaimRev_AFFIN
    ...
    try:
        ...
        for data in data_list:
            ...
            delete_query = """
                DELETE FROM tblDist_ClaimRev_AFFIN
